...
export default function TradeForm({ contact, type, onClose, onSuccess }: Props) {
    ...
    useEffect(() => {
        async function loadInitial() {
            setLoading(true);
            try {
                const [p, inv, accs] = await Promise.all([
                    getProducts({
    ...
    const handleSubmit = async (orderStatus?: 'pending' | 'delivered') => {
        ...
        if (isPaid && paymentMethod !== 'check' && !selectedAccountId) {
            toast.error('Lütfen kasa/banka seçimi yapınız.');
            return;
        }
        ...
        try {
            const result = await processTradeAction({
                type,
                contactId: contact.type === 'personnel' ? undefined : contact.id,
                employeeId: contact.type === 'personnel' ? contact.id : undefined,
                items,
                total,
                currency: 'TRY',
                description: description + ` (Debug: Type=${contact.type}, ID=${contact.id})`,
            ...
            if (result.success) {
                ...
                toast.error('İşlem başarısız: ' + ((result as any).error || (result as any).message || 'Bilinmeyen hata'));
            }
        } catch (error) {
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50/50 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-5 py-4 w-32 text-right">Toplam</th>
                            </tr>
                        </thead>
            ...
            <div className="bg-gray-900 rounded-3xl p-6 space-y-6 shadow-xl">
                <div className="flex justify-between items-end">
                    ...
                    <div className="flex flex-col gap-3 items-end">
                        {/* 1. Paid / Pending Toggle */}
                        <div className="flex items-center gap-2 bg-white/5 rounded-2xl p-1 border border-white/10">
                            <button
                                onClick={() => setIsPaid(false)}
                                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${!isPaid ? 'bg-amber-500 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                            >Veresiye (Açık)</button>
                            <button
                                onClick={() => setIsPaid(true)}
                                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${isPaid ? 'bg-emerald-500 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                            >Ödendi</button>
                        </div>

                        {/* 2. Payment Method (If Paid) */}
                        {isPaid && (
                            <div className="bg-white/5 rounded-2xl p-3 border border-white/10 w-full min-w-[200px]">
                                <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Ödeme Yöntemi</div>
                                <select
                                    value={paymentMethod}
                                    onChange={(e) => {
                        ...
                        {/* 3. Account Selection (If Paid & Not Check) */}
                        {isPaid && paymentMethod !== 'check' && (
                            <div className="bg-white/5 rounded-2xl p-3 border border-white/10 w-full min-w-[200px] animate-in slide-in-from-right-4">
                                <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">
                                    {paymentMethod === 'cash' && 'Kasa Seçimi'}
                                    {paymentMethod === 'eft' && 'Banka Hesabı'}
                                    {paymentMethod === 'credit_card' && 'POS Cihazı'}
                                </div>
                                <select
                                ...
                                >
                                    <option value="" className="bg-gray-800 text-gray-500">
                                        {financeAccounts.filter(a => {
                                            ...
                                            if (paymentMethod === 'credit_card') return a.type === 'pos';
                                            return false;
                                        }).length === 0 ? 'Lütfen hesap oluşturun' : 'Seçiniz...'}
                                    </option>
                                    {financeAccounts
                                        .filter(a => {
                                            if (paymentMethod === 'cash') return a.type === 'safe';
                        ...
                        {/* 4. Check Details (If Paid & Check) */}
                        {isPaid && paymentMethod === 'check' && (
                            <div className="bg-white/5 rounded-2xl p-3 border border-white/10 w-full min-w-[200px] space-y-2 animate-in slide-in-from-right-4">
                                <input
                                    type="text"
                                    placeholder="Banka Adı"
                                    value={checkDetails.bankName}
                                    onChange={e => setCheckDetails({ ...checkDetails, bankName: e.target.value })}
                                    className="w-full bg-black/20 text-white px-2 py-1 rounded text-xs outline-none"
                                />
                                <input
                                    type="text"
                                    placeholder="Çek No"
                    ...
                    {type === 'sale' ? (
                        ...
                        <div className="flex gap-3">
                            <button
                                onClick={() => handleSubmit('pending')}
                                disabled={loading || items.length === 0}
                                className="flex-1 bg-white/10 text-white py-4 rounded-2xl font-black text-xs hover:bg-white/15 transition-all disabled:opacity-50 border border-white/5"
                            >
                                📋 Sipariş Oluştur
                            </button>
                            <button
                                onClick={() => handleSubmit('delivered')}
                                disabled={loading || items.length === 0}
                                className="flex-[1.5] bg-emerald-500 text-white py-4 rounded-2xl font-black text-xs hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-900/40 disabled:opacity-50"
                            >
                                {loading ? 'İşleniyor...' : '📥 Teslim Al (Dükkana İndir)'}
                            </button>
                        </div>
                    )}
                </div>
