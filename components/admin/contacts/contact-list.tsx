...
export default function ContactList({ initialContacts, type }: Props) {
    const router = useRouter();
    const pathname = usePathname();
    const [contacts, setContacts] = useState(initialContacts);
    const [isMoreLoading, setIsMoreLoading] = useState(false);
    const [hasMore, setHasMore] = useState(initialContacts.length === 50);
    ...
    useEffect(() => {
        ...
        setHasMore(initialContacts.length === 50);
        setPage(1);
    }, [initialContacts]);
    ...
    async function handleLoadMore() {
        ...
        try {
            ...
            const nextBatch = await getContacts(type, 50, nextOffset);
    ...
    const getLabel = () => {
        ...
        if (type === 'subcontractor') return 'Taşeron';
        return 'Cari';
    }

    const getAddDrawerId = () => {
        if (type === 'customer') return 'add-customer';
        if (type === 'supplier') return 'add-supplier';
        // Needs a new drawer type or reuse generic? 
        // Existing system uses 'add-customer' / 'add-supplier'. 
        // I will need to update Drawer Mapping for 'add-subcontractor'.
        return `add-${type}`;
    }
        ...
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Link
                        href={`${pathname}?drawer=${getAddDrawerId()}`}
                ...
                <table className="w-full text-left">
                    <thead className="bg-gray-50/50">
                        <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                            <th className="px-8 py-4">{getLabel()} / Firma</th>
                                ...
                                <td className="px-8 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                router.push(`${pathname}?drawer=transaction&txType=collection&id=${c.id}`);
                                            }}
                                            className="p-2 hover:bg-emerald-50 rounded-xl text-emerald-600 transition-all inline-block group/btn"
                                            title="Tahsilat Ekle"
                                        >
                                        ...
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                router.push(`${pathname}?drawer=transaction&txType=payment&id=${c.id}`);
                                            }}
                                            className="p-2 hover:bg-red-50 rounded-xl text-red-600 transition-all inline-block group/btn"
                                            title="Ödeme Yap"
                                        >
                                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 10l7-7m0 0l7 7m-7-7v18" /></svg>
                                        </button>
                                        {/* Order Button (Generic for now) */}
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                // Ideally pass contact ID to order drawer
                                                router.push(`${pathname}?drawer=global-order`);
                                            }}
                                            className="p-2 hover:bg-indigo-50 rounded-xl text-indigo-600 transition-all inline-block group/btn"
                                        ...
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                router.push(`${pathname}?drawer=contact-detail&id=${c.id}`);
                                            }}
